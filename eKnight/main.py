import os, re, eQuestBrowsing, config, Tools.BrowserControl, Tools.PDFReader
...
def main():
    ...
    for AramarkInvoice in config.AramarkInvoices: # Goes through each document in the list of documents you placed
        ...
        if len(PotentialSRNumbers) > 1:
            input("Found 2 6-digit numbers that start with 3, check up on that")
        
        if PotentialSRNumbers == []: # If it's empty, put it in Failures folder
            os.rename(AramarkInvoice,AramarkInvoice.replace("\\Aramark Invoices\\","\\Aramark Invoices\\Failure\\"))
        else:
            ...
            FullSRNameMaybe = "SR" + SRNumAndAllAfter[0:SRNumAndAllAfter.find(" ")]
